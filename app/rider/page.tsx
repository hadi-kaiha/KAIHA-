...
fetchOrders(rd.id);
const ch=supabase.channel("orders").on("postgres_changes",{event:"INSERT",schema:"public",table:"orders"},(p)=>{
if(p.new.rider_id===rd.id){setOrders((o:any)=>[p.new,...o]); new Audio("https://cdn.pixabay.com/download/audio/2022/03/15/audio_62b4a0b9d3.mp3").play().catch(()=>{});}
}).subscribe();
...
await supabase.from("riders").update({status:newStatus,lat,lng}).eq("id",rider.id);
...
await supabase.from("riders").update({earnings:newEarn}).eq("id",rider.id);
}
};
