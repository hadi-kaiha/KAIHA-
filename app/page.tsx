...
const allProducts = {
  Fashion: [
    ...
    { id:6, name:"Street Shorts", price:1499, mrp:1999, sub:"Shorts", gender:"Boy", img:"https://images.unsplash.com/photo-1528832719520-7f2f6b7a377a?w=500&q=80", brand:"GEN Z DRIP" },
  ],
  Beauty: [
...
export default function Home(){
  ...
  const [live,setLive]=useState(19);
  ...
  return(
    <div className="bg-black text-white min-h-screen pb-36">
      <style>{`
        ...
        *{font-family:'Inter',sans-serif!important; letter-spacing:-0.2px}
        @keyframes float{0%,100%{transform:translateY(0)}50%{transform:translateY(-4px)}}
       .animate-float{animation:float 3s ease-in-out infinite}
      `}</style>

      {/* HEADER - SIRF KAIHA + LOGO */}
      <nav className="flex justify-between items-center p-4 sticky top-0 bg-black z-40 border-b border-zinc-900">
        <div className="flex items-center gap-3">
          <div className="w-11 h-11 rounded-xl overflow-hidden bg-black shadow-[0_0_20px_rgba(168,85,247,0.4)] animate-float">
            <img src="/kaiha-logo.png" alt="KAIHA" className="w-full h-full object-cover" />
          </div>
          <h1 className="font-black text-[24px] tracking-[0.2em] text-[#D4AF37]">KAIHA</h1>
        </div>
        <div className="flex gap-4 items-center">
          <button className="text-white"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M6 8a6 6 0 0 1 12 0c0 7 6 5 6 10H0c0-5 6-3 6-10"/><path d="M10 20a2 2 0 0 0 4 0"/></svg></button>
          <button className="text-white"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M20.8 4.6a5.5 5.5 0 0 0-7.8 0L12 5.6l-1-1a5.5 5.5 0 0 0-7.8 7.8l1 1L12 21l8.2-7.6 1-1a5.5 5.5 0 0 0 0-7.8z"/></svg></button>
          <button className="text-white"><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="8" r="4"/><path d="M4 20a8 8 0 0 1 16 0"/></svg></button>
      ...
      {/* CIRCLES - AB SARI PICS SHOW HONGI */}
      {activeTab==="Fashion" && (
          ...
          <div className="flex gap-4 overflow-x-auto px-2 pb-2">
            {fashionCircles.map(c=>(
              <button key={c.id} onClick={()=>setActiveCircle(c.name)} className="flex flex-col items-center gap-2 min-w-[72px]">
                <img src={c.img} className={`w-[72px] h-[72px] rounded-full object-cover object-top border-2 ${activeCircle===c.name?"border-yellow-500 scale-110":"border-zinc-700"} transition-all bg-zinc-800`} alt={c.name} />
      ...
      <div className="px-4 py-3 text-[11px] text-zinc-400 font-semibold flex gap-2 overflow-x-auto whitespace-nowrap border-b border-zinc-900">
      ...
      <div className="p-3 grid grid-cols-2 gap-3">
        {filtered.map(p=>(
          <div key={p.id} className="bg-[#121212] rounded-2xl overflow-hidden border border-zinc-900">
            <div className="relative"><img src={p.img} className="w-full h-[280px] object-cover object-top bg-zinc-800" alt="" /><span className="absolute bottom-2 left-2 bg-white text-black text-[10px] font-bold px-2 py-1 rounded-full">4.5 ★ | 1.2k</span></div>
            <div className="p-3">
              <h4 className="text-[11px] font-bold text-zinc-300">{p.brand}</h4>
              <h3 className="text-[13px] font-medium truncate">{p.name}</h3>
              <div className="flex items-center gap-2 mt-1"><span className="font-bold text-sm">Rs. {p.price}</span><span className="text-[11px] text-zinc-500 line-through">Rs. {p.mrp}</span><span className="text-[11px] text-green-400 font-bold">(40% OFF)</span></div>
      ...
      <div className="p-4">
        <div className="flex justify-between mb-3"><h2 className="font-bold flex items-center gap-2 text-[14px]"><span className="w-6 h-6 rounded-full border-2 border-red-500 flex items-center justify-center text-[10px]">▶</span> KAIHA TV</h2><span className="text-xs text-zinc-500">Shop from Videos</span></div>
        <div onClick={()=>setShowVideo(true)} className="bg-zinc-900 rounded-2xl overflow-hidden cursor-pointer border border-zinc-800">
          ...
          <div className="p-4 bg-[#1c1c1f] flex justify-between items-center"><div><h3 className="font-bold text-sm">3 Ways to Style This Kurta</h3><p className="font-bold mt-1">Rs. 1999</p></div><button className="bg-yellow-500 text-black font-bold px-5 py-2 rounded-full text-xs">Add to Cart</button></div>
        </div>
      </div>

      {showCart && (<div className="fixed inset-0 bg-black/70 z-[60] flex justify-end"><div className="bg-[#121212] w-[85%] max-w-sm h-full p-5"><div className="flex justify-between mb-6"><h2 className="font-bold text-xl">Smart Cart ({cart})</h2><button onClick={()=>setShowCart(false)} className="w-8 h-8 bg-zinc-800 rounded-full">✕</button></div><button onClick={()=>setShowCart(false)} className="mt-6 bg-yellow-500 text-black w-full py-3 rounded-xl font-bold">Checkout</button></div></div>)}
