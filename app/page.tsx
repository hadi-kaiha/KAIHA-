...
export default function Home() {
  ...
  useEffect(() => { const interval = setInterval(() => setShowLogo((prev) =>!prev), 2500); return () => clearInterval(interval); }, []);
  useEffect(()=>{
  ...
  const customerLogin = async()=>{
    ...
    if(data){ setCustomer(data); localStorage.setItem("kaiha_customer", JSON.stringify(data)); setShowLogin(false); }
    else {
      const {data:newC, error} = await supabase.from("kaiha_customers").insert({name:custName, gmail:custGmail}).select().single();
  ...
  const handleOrder = async(product:any)=>{
    ...
    const {error} = await supabase.from("kaiha_orders").insert({
      product_id: product.id, seller_id: product.seller_id, customer_name: customer.name, customer_gmail: customer.gmail,
      size: product.sizes?.[0] || 'M', color: product.colors?.[0] || 'Black', status: 'pending'
    });
    if(error) alert(error.message);
    else alert("Order sent to "+(product.kaiha_sellers?.shop_name || "Seller")+" ✅");
  }
  useEffect(()=>{
    ...
    <div className="bg-black text-white min-h-screen font-sans">
      <nav className="flex justify-between items-center p-4 border-b border-gray-800 sticky top-0 bg-black z-50">
        ...
        <div className="flex gap-4 items-center">
          {customer && <span className="text-[10px] bg-yellow-500 text-black px-2 py-1 rounded-full font-black">{customer.name}</span>}
      ...
      {sellerProducts.length > 0 && (<div className="px-4 py-4 border-t border-gray-800"><h2 className="text-xl font-black text-yellow-500">LIVE FROM SELLERS 🔴</h2><div className="grid grid-cols-2 gap-3 mt-3">{sellerProducts.map((p:any)=>(<div key={p.id} className="bg-[#101828] p-3 rounded-xl border border-yellow-500/20"><p className="font-bold text-sm">{p.name}</p><p className="text-yellow-500 text-sm font-bold">Rs. {p.price}</p><p className="text-[10px] text-gray-400">{p.kaiha_sellers?.shop_name} • Stock: {p.stock}</p><button onClick={()=>handleOrder(p)} className="w-full bg-yellow-500 text-black mt-2 p-2 rounded-xl font-black text-xs">{!customer? 'LOGIN TO ORDER' : 'ORDER NOW'}</button></div>))}</div></div>)}
      ...
      {showLogin && (<div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[200] p-4"><div className="bg-[#101828] p-6 rounded-2xl w-full max-w-sm border border-yellow-500/30"><h2 className="text-yellow-500 font-black text-xl">Customer Login 👤</h2><input value={custName} onChange={e=>setCustName(e.target.value)} placeholder="Your Name" className="w-full p-3 mt-3 rounded-xl bg-black border border-gray-700 text-white"/><input value={custGmail} onChange={e=>setCustGmail(e.target.value)} placeholder="Gmail" className="w-full p-3 mt-2 rounded-xl bg-black border border-gray-700 text-white"/><button onClick={customerLogin} className="w-full bg-yellow-500 text-black font-black p-3 rounded-xl mt-3">LOGIN & ORDER</button><button onClick={()=>setShowLogin(false)} className="w-full text-gray-400 mt-2 text-sm">Cancel</button></div></div>)}
      {showRiderAlert && liveRider && (<div className="fixed bottom-0 left-0 right-0 bg-[#101828] border-t-2 border-green-500 p-4 rounded-t-2xl shadow-2xl z-[100]"><div className="flex justify-between items-start"><div><h3 className="font-black text-green-400 text-lg">Your order is on the way! 🏍️</h3><p className="text-sm text-white mt-1">{liveRider.riderName} is coming</p></div><button onClick={()=>setShowRiderAlert(false)} className="text-gray-400 bg-gray-800 w-8 h-8 rounded-full">✕</button></div><div className="flex gap-2 mt-3"><a href="/track" className="flex-1 bg-yellow-500 text-black p-3 rounded-xl text-center font-black text-sm">View Location</a><a href={`tel:${liveRider.phone}`} className="flex-1 bg-white text-black p-3 rounded-xl text-center font-black text-sm">Call Rider</a></div></div>)}
