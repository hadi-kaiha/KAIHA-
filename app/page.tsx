...
export default function Home(){
  ...
  return(
    <div className="bg-black text-white min-h-screen pb-36">
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Syne:wght@800&display=swap');.kaiha{font-family:'Syne',sans-serif; background:linear-gradient(90deg,#ff3cac,#ffb347,#ff3cac); background-size:200% 200%; -webkit-background-clip:text; -webkit-text-fill-color:transparent; animation:grad 3s ease infinite} @keyframes grad{0%{background-position:0%}50%{background-position:100%}100%{background-position:0%}}`}</style>

      {/* HEADER - Professional Icons */}
      <nav className="flex justify-between items-center p-4 sticky top-0 bg-black/90 backdrop-blur z-40 border-b border-zinc-900">
        <h1 className="kaiha text-[34px] font-black tracking-tight">KAIHA</h1>
        <div className="flex gap-5 items-center">
          <button onClick={()=>alert("Notifications: No new notifications")}><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2"><path d="M6 8a6 6 0 0 1 12 0c0 7 6 5 6 10H0c0-5 6-3 6-10"/><path d="M10 20a2 2 0 0 0 4 0"/></svg></button>
          <button onClick={()=>setShowWishlist(true)}><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2"><path d="M20.8 4.6a5.5 5.5 0 0 0-7.8 0L12 5.6l-1-1a5.5 5.5 0 0 0-7.8 7.8l1 1L12 21l8.2-7.6 1-1a5.5 5.5 0 0 0 0-7.8z"/></svg></button>
          <button onClick={()=>setShowProfile(true)}><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2"><circle cx="12" cy="8" r="4"/><path d="M4 20a8 8 0 0 1 16 0"/></svg></button>
          <button onClick={()=>setShowCart(true)} className="relative"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2"><circle cx="8" cy="21" r="1"/><circle cx="19" cy="21" r="1"/><path d="M2.5 3h2l2.5 12h13l2-8H6.5"/></svg>{cart>0 && <span className="absolute -top-2 -right-2 bg-yellow-500 text-black text-[10px] font-bold w-4 h-4 rounded-full flex items-center justify-center">{cart}</span>}</button>
        </div>
      </nav>
      ...
      {/* SMART BAR */}
      <div className="px-4 py-3 text-[11px] text-zinc-400 font-bold flex gap-2 overflow-x-auto whitespace-nowrap border-b border-zinc-900">
        ...
        {filtered.map(p=>(
          <div key={p.id} className="bg-[#121212] rounded-2xl overflow-hidden border border-zinc-900">
            <div className="relative">
              ...
              <span className="absolute bottom-2 left-2 bg-white text-black text-[10px] font-black px-2 py-1 rounded-full">4.5 ★ | 1.2k</span>
            </div>
            <div className="p-3">
              <h4 className="text-[11px] font-black text-zinc-300">{p.brand}</h4>
              <h3 className="text-[13px] truncate">{p.name}</h3>
              <p className="text-[11px] text-zinc-500">{p.sub} • {p.gender}</p>
              <div className="flex items-center gap-2 mt-1">
                <span className="font-black text-sm">Rs. {p.price}</span>
              ...
              <button onClick={()=>setCart(c=>c+1)} className="mt-2 w-full bg-white text-black text-[11px] font-black py-2 rounded-full">ADD TO CART +10 Coins</button>
            </div>
          </div>
        ))}
      </div>
      ...
      {showCart && (
        <div className="fixed inset-0 bg-black/70 z-[60] flex justify-end">
          <div className="bg-[#121212] w-[85%] max-w-sm h-full p-5 animate-in">
            ...
            <div className="space-y-3 text-sm"><p>✓ Coins: {cart*10} Earned</p><p>✓ Coupon KAIHA1 -20%</p><p>✓ Black Card Active</p><p>✓ Complete Look AI</p><p>✓ AI Size: M</p><p>✓ Flash Sale 59:30</p></div>
            <button onClick={()=>setShowCart(false)} className="mt-6 bg-yellow-500 text-black w-full py-3 rounded-xl font-black">Checkout - Rs. {cart*1999}</button>
          </div>
        </div>
      )}

      {showWishlist && (
        <div className="fixed inset-0 bg-black/70 z-[60] flex justify-end"><div className="bg-[#121212] w-[85%] max-w-sm h-full p-5"><div className="flex justify-between mb-6"><h2 className="font-black text-xl">Wishlist ❤️</h2><button onClick={()=>setShowWishlist(false)} className="w-8 h-8 bg-zinc-800 rounded-full">✕</button></div><p className="text-zinc-400 text-sm">3 items in wishlist</p></div></div>
      )}

      {showProfile && (
        <div className="fixed inset-0 bg-black/70 z-[60] flex justify-end"><div className="bg-[#121212] w-[85%] max-w-sm h-full p-5"><div className="flex justify-between mb-6"><h2 className="font-black text-xl">Profile</h2><button onClick={()=>setShowProfile(false)} className="w-8 h-8 bg-zinc-800 rounded-full">✕</button></div><p className="text-sm">HADI • 60 Coins • Black Card Member</p></div></div>
      )}

      {showVideo && (
        <div className="fixed inset-0 bg-black z-[70] flex flex-col">
          <div className="p-4 flex justify-between"><h3 className="font-black">KAIHA TV</h3><button onClick={()=>setShowVideo(false)} className="w-8 h-8 bg-zinc-800 rounded-full">✕</button></div>
