...
{tvReels.length===0? reels.map((r:any)=>(
...
<div style={{position:"absolute",bottom:0,left:0,right:0,padding:"20px 16px 100px 16px",background:"linear-gradient(to top, rgba(0,0,0,0.9), transparent)"}}>
<div style={{fontSize:"13px",fontWeight:"700"}}>@{r.shop}</div>
<div style={{fontSize:"11px",color:"#ccc"}}>{r.desc}</div>
</div>
<div style={{position:"absolute",right:"12px",bottom:"160px",display:"flex",flexDirection:"column",gap:"18px",alignItems:"center"}}>
<button onClick={()=>likeReel(r.id)} style={{background:"rgba(255,255,255,0.18)",border:"none",width:"48px",height:"48px",borderRadius:"999px",color:r.liked?"#ff3040":"#fff",fontSize:"22px"}}>♥</button>
<div style={{fontSize:"11px",color:"#fff",marginTop:"-14px"}}>{r.likes}</div>
</div>
</div>
)) : tvReels.map((r:any)=>(
<div key={r.id} style={{width:"100%",height:"100vh",position:"relative",scrollSnapAlign:"start",background:"#000"}}>
...
<input value={cmt[r.id]||""} onChange={e=>setCmt({...cmt,[r.id]:e.target.value})} placeholder="Comment..." style={{flex:1,background:"rgba(0,0,0,0.6)",border:"1px solid #333",borderRadius:"999px",padding:"7px 10px",color:"#fff",fontSize:"10px"}}/>
<button onClick={()=>{ if(!cmt[r.id]) return; alert("Comment: "+cmt[r.id]); setCmt({...cmt,[r.id]:""}); }} style={{background:"#D4B78F",color:"#000",border:"none",borderRadius:"999px",padding:"7px 12px",fontSize:"10px",fontWeight:"800"}}>Post</button>
</div>
</div>
...
{showOptions&&<div style={{position:"fixed",inset:0,zIndex:300,background:"rgba(0,0,0,0.88)",display:"flex",alignItems:"flex-end",justifyContent:"center"}}><div style={{background:"#141414",width:"100%",maxWidth:"390px",borderRadius:"24px 24px 0 0",padding:"18px",borderTop:"1px solid #D4B78F44"}}><div style={{display:"flex",gap:"12px"}}><img src={showOptions.im} style={{width:"70px",height:"70px",borderRadius:"12px"}}/><div><div style={{fontSize:"14px",fontWeight:"700"}}>{showOptions.n}</div><div style={{fontSize:"12px",color:"#D4B78F"}}>{PKR(showOptions.pr)}</div></div><button onClick={()=>setShowOptions(null)} style={{marginLeft:"auto",background:"#222",border:"none",color:"#fff",width:"30px",height:"30px",borderRadius:"999px"}}>✕</button></div><div style={{marginTop:"14px"}}><div style={{fontSize:"10px",color:"#D4B78F"}}>SIZE</div><div style={{display:"flex",gap:"8px",marginTop:"8px",flexWrap:"wrap"}}>{(showOptions.sizes||["M","L","XL"]).map((s:any)=><button key={s} onClick={()=>setSelSize(s)} style={{background:selSize===s?"#D4B78F":"#0a0a0a",color:selSize===s?"#000":"#fff",border:"1px solid #333",borderRadius:"999px",padding:"8px 14px",fontSize:"11px"}}>{s}</button>)}</div></div><div style={{marginTop:"12px"}}><div style={{fontSize:"10px",color:"#D4B78F"}}>COLOR</div><div style={{display:"flex",gap:"8px",marginTop:"8px"}}>{(showOptions.colors||["#000","#fff"]).map((c:any)=><button key={c} onClick={()=>setSelColor(c)} style={{background:c,border:selColor===c?"2px solid #D4B78F":"1px solid #444",width:"32px",height:"32px",borderRadius:"999px"}}/>)}</div></div><div style={{marginTop:"12px",display:"flex",alignItems:"center",justifyContent:"space-between"}}><div style={{fontSize:"10px",color:"#D4B78F"}}>QUANTITY</div><div style={{display:"flex",alignItems:"center",gap:"12px"}}><button onClick={()=>setSelQty(Math.max(1,selQty-1))} style={{width:"34px",height:"34px",borderRadius:"999px",background:"#0a0a0a",border:"1px solid #333",color:"#fff"}}>-</button><span style={{fontSize:"14px",fontWeight:"800"}}>{selQty}</span><button onClick={()=>setSelQty(selQty+1)} style={{width:"34px",height:"34px",borderRadius:"999px",background:"#0a0a0a",border:"1px solid #333",color:"#fff"}}>+</button></div></div><button onClick={confirmAdd} style={{width:"100%",marginTop:"16px",background:"#D4B78F",color:"#000",border:"none",padding:"14px",borderRadius:"999px",fontWeight:"800"}}>ADD TO BAG - {PKR(showOptions.pr*selQty)}</button></div></div>}
{showCheckout&&<div style={{position:"fixed",inset:0,zIndex:400,background:"rgba(0,0,0,0.94)",display:"flex",alignItems:"center",justifyContent:"center",padding:"12px"}}><div style={{background:"#141414",width:"100%",maxWidth:"370px",borderRadius:"18px",padding:"16px",border:"1px solid #D4B78F44",maxHeight:"90vh",overflowY:"auto"}}><div style={{display:"flex",justifyContent:"space-between"}}><b>Checkout {PKR(tot)}</b><button onClick={()=>setShowCheckout(false)} style={{background:"#222",border:"none",color:"#fff",width:"28px",height:"28px",borderRadius:"999px"}}>✕</button></div><div style={{marginTop:"12px"}}><input value={buyerName} onChange={e=>setBuyerName(e.target.value)} placeholder="Full Name" style={{width:"100%",background:"#0a0a0a",border:"1px solid #333",borderRadius:"8px",padding:"10px",color:"#fff"}}/></div><div style={{marginTop:"8px",display:"flex",gap:"6px"}}><input value={buyerPhone} onChange={e=>setBuyerPhone(e.target.value)} placeholder="03XX-XXXXXXX" style={{flex:1,background:"#0a0a0a",border:"1px solid #333",borderRadius:"8px",padding:"10px",color:"#fff"}}/><button onClick={sendPhoneOtp} style={{background:"#fff",color:"#000",border:"none",borderRadius:"8px",padding:"10px"}}>OTP</button></div>{phoneOtpSent&&<input value={phoneOtp} onChange={e=>setPhoneOtp(e.target.value)} placeholder="OTP 123456" style={{width:"100%",background:"#0a0a0a",border:"1px solid #D4B78F",borderRadius:"8px",padding:"10px",color:"#fff",marginTop:"6px"}}/>}<div style={{marginTop:"8px",display:"flex",gap:"6px"}}><button onClick={getCurrentLoc} style={{flex:1,background:"#111",border:"1px solid #D4B78F44",color:"#D4B78F",borderRadius:"8px",padding:"10px",fontSize:"10px"}}>📍 Current Location</button><button onClick={()=>setLocText(prompt("Address")||"")} style={{flex:1,background:"#111",border:"1px solid #333",color:"#fff",borderRadius:"8px",padding:"10px",fontSize:"10px"}}>✏️ Select Location</button></div>{locText&&<div style={{fontSize:"10px",color:"#4CAF50",marginTop:"4px"}}>✅ {locText}</div>}<div style={{marginTop:"8px",display:"flex",gap:"8px"}}><button onClick={()=>setPayType("COD")} style={{flex:1,background:payType==="COD"?"#D4B78F":"#0a0a0a",color:payType==="COD"?"#000":"#fff",border:"1px solid #333",borderRadius:"10px",padding:"12px",fontSize:"10px"}}>COD</button><button onClick={()=>setPayType("ONLINE")} style={{flex:1,background:payType==="ONLINE"?"#D4B78F":"#0a0a0a",color:payType==="ONLINE"?"#000":"#fff",border:"1px solid #333",borderRadius:"10px",padding:"12px",fontSize:"10px"}}>Online 03320821575</button></div><button onClick={placeOrder} disabled={loading} style={{width:"100%",marginTop:"14px",background:"#4CAF50",color:"#fff",border:"none",padding:"14px",borderRadius:"999px",fontWeight:"800"}}>{loading?"PLACING...":`PLACE ORDER ${PKR(tot)} →`}</button></div></div>}
