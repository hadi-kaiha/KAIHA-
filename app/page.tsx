...
import { ShoppingCart, X, Plus, Minus, Mic, Search, Bell, Heart, User, PlayCircle } from "lucide-react";
...
export default function Home() {
  ...
  const [live,setLive]=useState(12); const [qty,setQty]=useState(1);
  const [coupon,setCoupon]=useState(""); const [discount,setDiscount]=useState(0);
  const [activeVideo,setActiveVideo]=useState<any>(null);
  useEffect(() => {
    const i1 = setInterval(() => setShowLogo((p)=>!p), 2500);
    const i2 = setInterval(()=>setLive(c=>Math.max(8,c+(Math.random()>0.5?1:-1))),3000);
    if(typeof window!=="undefined" &&!localStorage.getItem("login")){localStorage.setItem("login","1"); localStorage.setItem("coins","60");}
  ...
  return (
    <div className="bg-black text-white min-h-screen font-sans">
      <nav className="flex justify-between items-center p-4 border-b border-gray-800 sticky top-0 bg-black z-50"><div className="relative w-32 h-10 flex items-center"><AnimatePresence mode="wait">{showLogo? (<motion.div key="logo" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="relative w-full h-9"><Image src="/IMG-20200913-WA5443.jpg" alt="KAIHA Logo" fill className="object-contain" /></motion.div>) : (<motion.h1 key="text" className="text-2xl font-bold tracking-widest bg-gradient-to-r from-purple-500 to-yellow-500 bg-clip-text text-transparent" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>KAIHA</motion.h1>)}</AnimatePresence></div><div className="flex gap-4"><Bell className="w-6 h-6" /><Heart className="w-6 h-6" /><User onClick={()=>router.push('/profile')} className="w-6 h-6" /><button onClick={()=>setCartOpen(true)} className="relative"><ShoppingCart className="w-6 h-6" />{cart.length>0 && <span className="absolute -top-2 -right-2 bg-red-500 rounded-full w-5 h-5 text-xs flex items-center justify-center font-bold">{cart.length}</span>}</button></div></nav>
      ...
      <div className="p-3 bg-black"><div className="flex items-center bg-gray-900 rounded-full px-4 py-2"><Search className="w-5 h-5 text-gray-400" /><input placeholder="Search mic se bolo..." className="bg-transparent outline-none flex-1 ml-2 text-sm" id="searchInput"/><Mic className="w-5 h-5 text-gray-400 cursor-pointer" onClick={()=>{const SR=(window as any).webkitSpeechRecognition||(window as any).SpeechRecognition; if(SR){const r=new SR(); r.lang="en-US"; r.start(); r.onresult=(e:any)=>{ (document.getElementById("searchInput") as any).value=e.results[0][0].transcript;}} }} /></div></div>
      ...
      <div className="px-4 py-4 border-t border-gray-800"><div className="flex items-center justify-between mb-3"><h2 className="text-xl font-bold flex items-center gap-2"><PlayCircle className="w-6 h-6 text-red-500" /> KAIHA TV</h2><p className="text-xs text-gray-400">Tap to Watch</p></div><div className="flex gap-3 overflow-x-auto pb-2">{filteredVideos.map(video=>(<div key={video.id} onClick={()=>setActiveVideo(video)} className="min-w-[280px] bg-gray-900 rounded-xl overflow-hidden cursor-pointer"><div className="relative w-full h-40"><Image src={video.thumbnail} alt={video.title} fill className="object-cover" unoptimized /><PlayCircle className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-12 h-12 text-white opacity-80" /></div><div className="p-3"><p className="font-semibold text-sm">{video.title}</p><div className="flex items-center justify-between mt-2"><div><p className="text-xs text-gray-400">{video.product}</p><p className="text-sm font-bold">Rs. {video.price}</p></div><button onClick={(e)=>{e.stopPropagation(); setCart([...cart, {name: video.product, price: video.price}])}} className="bg-yellow-500 text-black text-xs px-3 py-1.5 rounded-full font-bold">Add to Cart</button></div></div></div>))}</div></div>
      <div className="p-6"><p className="text-gray-400 text-sm mb-2">Colors: Black, Gold • Complete The Look -15%</p><div className="relative"><motion.div animate={{ y: [0, -10, 0] }} transition={{ repeat: Infinity, duration: 3 }} className="w-60 mx-auto"><Image src={currentProduct.img} alt={currentProduct.name} width={240} height={360} className="rounded-lg" unoptimized /></motion.div></div><h2 className="text-center text-xl mt-4">{currentProduct.name}</h2><p className="text-center text-gray-400">Rs. {currentProduct.price}</p><div className="flex justify-center items-center gap-3 mt-3"><button onClick={()=>setQty(q=>Math.max(1,q-1))} className="w-8 h-8 border rounded-full"><Minus className="w-4 h-4 mx-auto"/></button><span>{qty}</span><button onClick={()=>setQty(q=>q+1)} className="w-8 h-8 border rounded-full"><Plus className="w-4 h-4 mx-auto"/></button></div><p className="text-center text-xs mt-3">1 Refer=20 Coins • Black Card 1000 coins • Coins: {typeof window!=="undefined"?localStorage.getItem("coins"):"60"}</p><button onClick={()=>setCart([...cart, currentProduct])} className="w-full bg-yellow-500 text-black mt-4 p-3 rounded-full font-bold">Add to Cart</button></div>
      {cartOpen && (<div className="fixed top-0 right-0 w-80 h-full bg-gray-900 p-4 z-50 overflow-y-auto"><X onClick={()=>setCartOpen(false)} className="mb-4 cursor-pointer"/><h2 className="text-xl mb-4">Cart ({cart.length})</h2>{cart.map((item,i)=><p key={i} className="text-sm">{item.name} - Rs.{item.price}</p>)}<div className="flex gap-2 mt-4"><input onChange={e=>setCoupon(e.target.value)} placeholder="KAIHA1" className="flex-1 bg-black p-2 rounded-full text-xs"/><button onClick={()=>{if(coupon==="KAIHA1"){setDiscount(200); alert("200 OFF!"); if(typeof window!=="undefined" &&!localStorage.getItem("first_order_done")){localStorage.setItem("first_order_done","1"); const c=parseInt(localStorage.getItem("coins")||"0")+90; localStorage.setItem("coins",c.toString()); alert("90 COINS!");}} }} className="bg-white text-black px-4 rounded-full text-xs">Apply</button></div><p className="text-xs mt-2">Discount: {discount} • COD | JazzCash | Easypaisa</p><button onClick={()=>{if(confirm("Cancel?")) setCart([])}} className="w-full border border-red-500 text-red-500 py-2 rounded-full mt-4 text-xs">Cancel Order</button><button onClick={()=>router.push('/cart')} className="w-full bg-yellow-500 text-black py-2 rounded-full mt-3 font-bold">Checkout</button></div>)}
      ...
      <footer className="text-center py-10 text-gray-500 text-sm">KAIHA © 2026 - HADI</footer>
    </div>
  );
  }
