...
const fetchLive=async()=>{
 ...
 let localLive:any[]=[];
 if(sp){localLive=JSON.parse(sp).filter((p:any)=>p.isOpen!==false&&(p.stock===undefined||p.stock>0)).map((p:any)=>({id:p.id,n:p.name,pr:p.price||p.pr,cat:p.category||"FASHION",sub:p.subcategory||p.sub||"MALE",im:p.image_url||p.image||p.im,colors:p.colors||[],shopName:p.shop_name||"Local"}));}
...
<div style={{color:"#D4B78F",fontSize:"10px",letterSpacing:"0.35em",fontWeight:"700"}}>NEW ARRIVALS • {sellerProds.length} LIVE</div>
