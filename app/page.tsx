...
const fetchLive=async()=>{
 try{
  ...
  const mapped=(data||[]).map((d:any)=>({
   ...
   n:d.name,
   pr:d.price,
   cat:(d.category||"FASHION").toUpperCase(),
   sub:(d.subcategory||"MALE").toUpperCase(),
   im:d.image_url,
   colors:d.colors||[],
   shopName:d.shop_name||"Seller Shop",
   shopCat:(d.category||"FASHION").toUpperCase()
  }));
  setSellerProds(mapped);
  console.log("LIVE SELLER:",mapped.length);
...
{[{l:"Home",v:"home"},{l:"Shop",v:"category"},{l:"KAIHA TV",v:"reels"},{l:"Bag",v:"bag"},{l:"Profile",v:"profile"}].map((it:any)=><div key={it.l} onClick={()=>{setPg(it.v);setMn(false)}} style={{padding:"14px",borderRadius:"12px",border:"1px solid #222",marginTop:"10px",color:pg===it.v?"#D4B78F":"#ccc",display:"flex",justifyContent:"space-between"}}><span>{it.l}</span><span>→</span></div>)}
<div style={{marginTop:"20px",background:"#141414",borderRadius:"12px",padding:"12px",border:"1px solid #222"}}><div style={{fontSize:"10px",color:"#D4B78F"}}>SELLER LIVE</div><div style={{fontSize:"12px",marginTop:"4px"}}>{sellerProds.length} Products in Supabase</div></div>
</div></div></div>}
...
<div style={{color:"#D4B78F",fontSize:"10px",letterSpacing:"0.35em",fontWeight:"700"}}>NEW • {sellerProds.length} SELLER LIVE</div>
...
{pg==="reels"&&<div className="page"><div style={{padding:"12px 16px",display:"flex",justifyContent:"space-between"}}><b style={{color:"#D4B78F"}}>KAIHA TV • {reels.length} + {sellerProds.length} LIVE</b><label style={{background:"#D4B78F",color:"#000",borderRadius:"999px",padding:"8px 14px",fontSize:"10px",fontWeight:"800"}}> + UPLOAD<input type="file" accept="image/*,video/*" style={{display:"none"}} onChange={(e:any)=>{const f=e.target.files[0];if(!f)return;const r=new FileReader();r.onload=()=>{const newReel={id:Date.now(),shop:user?.name||"You",likes:0,im:r.result as string,desc:"My New Reel"};const upd=[newReel,...reels];setReels(upd);localStorage.setItem("kaiha_reels",JSON.stringify(upd));};r.readAsDataURL(f);}}/></label></div>{reels.map((reel:any)=><div key={reel.id} style={{background:"#141414",margin:"10px 16px",borderRadius:"14px",overflow:"hidden",border:"1px solid #222"}}><img src={reel.im} style={{width:"100%",height:"320px",objectFit:"cover"}}/><div style={{padding:"10px",display:"flex",justifyContent:"space-between"}}><div><div style={{fontSize:"12px"}}>@{reel.shop}</div><div style={{fontSize:"9px",color:"#888"}}>{reel.desc}</div></div><button onClick={()=>likeReel(reel.id)} style={{background:"none",border:"none",color:reel.liked?"#ff3040":"#fff"}}>{reel.liked?"♥":"♡"} {reel.likes}</button></div></div>)}</div>}

{pg==="category"&&<div className="page" style={{padding:"16px"}}><div style={{display:"flex",justifyContent:"space-between"}}><b>{mainCat} {mainCat==="FASHION"?`- ${subCat}`:""} ({filtered.length}) Seller:{sellerProds.length}</b><span onClick={()=>setPg("home")} style={{color:"#D4B78F",fontSize:"12px"}}>← Back</span></div><div style={{display:"flex",gap:"8px",marginTop:"14px",overflowX:"auto"}}>{["FASHION","BEAUTY","HOME"].map(c=><button key={c} onClick={()=>setMainCat(c)} style={{background:mainCat===c?"#D4B78F":"#141414",color:mainCat===c?"#000":"#fff",border:"1px solid #333",borderRadius:"999px",padding:"8px 16px",fontSize:"10px",fontWeight:"700"}}>{c}</button>)}</div>{mainCat==="FASHION"&&<div style={{display:"flex",gap:"7px",marginTop:"12px",overflowX:"auto"}}>{["MALE","FEMALE","BOY","GIRL","UNISEX"].map(s=><button key={s} onClick={()=>setSubCat(s)} style={{background:subCat===s?"#fff":"#222",color:subCat===s?"#000":"#aaa",border:"1px solid #333",borderRadius:"999px",padding:"7px 14px",fontSize:"9px"}}>{s}</button>)}</div>}<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"12px",marginTop:"16px"}}>{filtered.map((p:any)=><div key={p.id} style={{background:"#141414",borderRadius:"18px",padding:"8px",border:"1px solid #222"}}><div style={{position:"relative"}}><img src={p.im} style={{width:"100%",height:"145px",borderRadius:"12px",objectFit:"cover"}}/>{p.colors?.length>0&&<div style={{position:"absolute",bottom:"6px",right:"6px",display:"flex",gap:"2px"}}>{p.colors.slice(0,4).map((c:any)=><div key={c} style={{width:9,height:9,borderRadius:999,background:c,border:"1px solid #fff"}}/>)}</div>}</div><div style={{fontSize:"11px",marginTop:"8px",fontWeight:"700"}}>{p.n} {p.shopName&&`•`}</div><div style={{fontSize:"9px",color:"#D4B78F"}}>{p.shopName||p.sub}</div><div style={{fontSize:"10px",color:"#D4B78F",marginTop:"3px"}}>{PKR(p.pr)}</div><button onClick={()=>add(p)} style={{width:"100%",marginTop:"8px",background:"#D4B78F",border:"none",borderRadius:"999px",padding:"8px",fontSize:"10px",fontWeight:"800"}}>ADD TO BAG</button></div>)}</div></div>}
...
<div style={{display:"flex",gap:"10px",alignItems:"center"}}><img src={LOGO} style={{width:"38px",height:"38px",borderRadius:"10px"}}/><div><div style={{fontSize:"10px",color:"#D4B78F",letterSpacing:"0.3em"}}>KAIHA BLACK</div><div style={{fontSize:"11px",fontWeight:"800"}}>{user?.name||"Guest"}</div></div><div style={{marginLeft:"auto",textAlign:"right"}}><div style={{fontSize:"9px",color:"#888"}}>COINS</div><div style={{color:"#D4B78F",fontWeight:"800"}}>{user?.coins||0}</div></div></div>
<div style={{marginTop:"12px",display:"flex",gap:"8px"}}><div style={{background:"#0a0a0a",border:"1px solid #333",borderRadius:"8px",padding:"8px",flex:1}}><div style={{fontSize:"8px",color:"#888"}}>LOGIN ID</div><div style={{fontSize:"10px",fontWeight:"700"}}>{user?.loginId||"KAIHA-00000"}</div></div><div style={{background:"#0a0a0a",border:"1px solid #333",borderRadius:"8px",padding:"8px",flex:1}}><div style={{fontSize:"8px",color:"#888"}}>LIVE PRODUCTS</div><div style={{fontSize:"10px",fontWeight:"700"}}>{sellerProds.length} in DB</div></div></div>
</div>
<div style={{marginTop:"16px"}}><b>Accounts ({accounts.length})</b>{accounts.map((a:any,i:number)=><div key={i} onClick={()=>{localStorage.setItem("kaiha_user",JSON.stringify(a));setUser(a)}} style={{background:user?.id===a.id?"#1a1a1a":"#141414",border:"1px solid #222",borderRadius:"10px",padding:"10px",marginTop:"8px",display:"flex",justifyContent:"space-between"}}><div><div style={{fontSize:"12px"}}>{a.name} • {a.coins||0}c</div><div style={{fontSize:"9px",color:"#888"}}>{a.gmail}</div></div><div style={{fontSize:"10px",color:user?.id===a.id?"#D4B78F":"#666"}}>{user?.id===a.id?"Active":"Switch"}</div></div>)}</div>
