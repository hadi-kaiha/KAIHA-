...
@keyframes kaihaIn{0%{max-width:0;opacity:0;transform:translateX(-12px)}100%{max-width:135px;opacity:1;transform:translateX(0)}}
...
.kaihaOnce{overflow:hidden;white-space:nowrap;max-width:0;opacity:0;animation:kaihaIn 0.9s cubic-bezier(.2,.8,.2,1) forwards;}
`}</style>
...
{/* HEADER - KAIHA HADI SUKKUR 2026 */}
<div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"12px 18px",borderBottom:"1px solid #1a1a1a",position:"sticky",top:0,background:"rgba(10,10,10,0.96)",backdropFilter:"blur(12px)",zIndex:20}}>
<div onClick={()=>setPg("home")} style={{display:"flex",alignItems:"center",gap:"12px"}}>
<img src={LOGO} alt="K" style={{height:"52px",width:"52px",objectFit:"contain",borderRadius:"12px"}}/>
<div style={{display:"flex",flexDirection:"column"}}>
<span key={pg} className={pg==="home"?"kaihaOnce":""} style={{color:"#D4B78F",letterSpacing:"0.38em",fontSize:"18px",fontFamily:"'Montserrat', sans-serif",fontWeight:"400",lineHeight:1}}>KAIHA</span>
<span style={{color:"#6B6B6B",letterSpacing:"0.22em",fontSize:"8px",fontFamily:"'Montserrat', sans-serif",fontWeight:"500",marginTop:"3px"}}>HADI SUKKUR • 2026</span>
</div>
</div>
...
<div style={{display:"flex",gap:"12px",marginTop:"12px",overflowX:"auto",padding:"0 16px 8px"}}>{P.map((s:any,i:number)=><div key={s.id} onClick={()=>setPg("all")} style={{minWidth:"60px",textAlign:"center",animation:`fadeUp 0.5s ease ${i*0.08}s both`}}><div style={{width:"56px",height:"56px",borderRadius:"999px",overflow:"hidden",border:"1.5px solid #D4B78F88",margin:"0 auto"}}><img src={s.im} style={{width:"100%",height:"100%",objectFit:"cover"}}/></div><div style={{fontSize:"9px",marginTop:"6px",color:"#aaa"}}>{s.n}</div></div>)}</div>
...
{pg==="bag"&&<div className="page" style={{padding:"16px"}}><b>Bag ({cart.length}) ${tot}</b>{cart.length===0?<div style={{color:"#888",marginTop:"30px",textAlign:"center"}}>Khali hai</div>:<div style={{marginTop:"14px",display:"grid",gap:"10px"}}>{cart.map((c:any,i:number)=><div key={c.aid} style={{background:"#141414",borderRadius:"12px",padding:"12px",display:"flex",gap:"12px",alignItems:"center",border:"1px solid #222"}}><img src={c.im} style={{width:"54px",height:"54px",borderRadius:"10px"}}/><div style={{flex:1,fontSize:"12px"}}>{c.n}</div><button onClick={()=>setCart(cart.filter((_:any,idx:number)=>idx!==i))} style={{border:"1px solid #333",background:"none",color:"#888",borderRadius:"999px",width:"28px",height:"28px"}}>✕</button></div>)}</div>}</div>}

{pg==="profile"&&<div className="page" style={{padding:"24px",textAlign:"center"}}><div style={{width:"80px",height:"80px",borderRadius:"999px",background:"#222",margin:"0 auto",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"32px",border:"1px solid #D4B78F44"}}>H</div><div style={{marginTop:"12px",fontWeight:"700",letterSpacing:"0.1em"}}>HADI</div><div style={{color:"#D4B78F",fontSize:"9px",letterSpacing:"0.2em",marginTop:"4px"}}>SUKKUR • FOUNDER • 2026</div><div style={{color:"#888",fontSize:"11px",marginTop:"6px"}}>hadi@kaiha.com</div><div style={{marginTop:"20px",color:"#555",fontSize:"10px",letterSpacing:"0.1em"}}>© KAIHA - HADI SUKKUR 2026</div></div>}

{pg==="all"&&<div className="page" style={{padding:"16px"}}><div style={{display:"flex",justifyContent:"space-between"}}><b>All Items</b><span onClick={()=>setPg("home")} style={{color:"#D4B78F",fontSize:"12px"}}>← Back</span></div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"12px",marginTop:"12px"}}>{P.map((p:any)=><div key={p.id} className="card" style={{background:"#141414",borderRadius:"12px",padding:"8px",border:"1px solid #222"}}><img src={p.im} style={{width:"100%",height:"130px",borderRadius:"10px",objectFit:"cover"}}/><div style={{fontSize:"11px",marginTop:"6px"}}>{p.n}</div><button onClick={()=>add(p)} style={{width:"100%",marginTop:"6px",border:"1px solid #D4B78F88",background:"transparent",color:"#D4B78F",borderRadius:"999px",padding:"7px",fontSize:"9px"}}>ADD</button></div>)}</div></div>}

{pg==="reels"&&<div style={{height:"70vh",overflowY:"scroll"}}>{P.map((p:any)=><div key={p.id} style={{height:"65vh",position:"relative",margin:"8px 12px",borderRadius:"18px",overflow:"hidden",background:"#111"}}><img src={p.im} style={{width:"100%",height:"100%",objectFit:"cover"}}/><div style={{position:"absolute",bottom:"20px",left:"18px"}}><b>{p.n}</b><div style={{color:"#D4B78F"}}>${p.pr}</div><button onClick={()=>add(p)} style={{marginTop:"8px",background:"#fff",color:"#000",border:"none",padding:"8px 16px",borderRadius:"999px",fontSize:"11px",fontWeight:"700"}}>Add to Bag</button></div></div>)}</div>}

{/* FINAL COPYRIGHT FOOTER - HADI SUKKUR 2026 */}
<div style={{textAlign:"center",padding:"28px 0 14px",borderTop:"1px solid #1a1a1a",marginTop:"20px"}}>
<div style={{color:"#D4B78F",fontSize:"11px",letterSpacing:"0.32em",fontFamily:"'Montserrat', sans-serif",fontWeight:"400"}}>KAIHA</div>
<div style={{color:"#666",fontSize:"8.5px",letterSpacing:"0.18em",fontFamily:"'Montserrat', sans-serif",marginTop:"6px"}}>© KAIHA - HADI SUKKUR 2026</div>
<div style={{color:"#3a3a3a",fontSize:"7px",letterSpacing:"0.15em",marginTop:"4px"}}>LUXURY FROM SUKKUR • PAKISTAN</div>
</div>
...
<div onClick={()=>setPg("reels")} style={{color:pg==="reels"?"#D4B78F":"#6B6B6B",display:"flex",flexDirection:"column",alignItems:"center",gap:"5px"}}><svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={pg==="reels"?2:1.6}><polygon points="6 3 20 12 6 21 6 3"/></svg><span style={{fontSize:"9px"}}>Reels</span></div>
...
{mn&&<div style={{position:"fixed",inset:0,zIndex:99,display:"flex",justifyContent:"flex-end"}}><div style={{position:"absolute",inset:0,background:"rgba(0,0,0,0.75)"}} onClick={()=>setMn(false)}/><div style={{position:"relative",width:"78%",background:"#0a0a0a",height:"100%",padding:"18px",borderLeft:"1px solid #D4B78F33",animation:"slideMenu 0.38s ease"}}><div style={{display:"flex",alignItems:"center",gap:"8px"}}><img src={LOGO} style={{height:"44px",width:"44px",borderRadius:"10px"}}/><div><div style={{color:"#D4B78F",letterSpacing:"0.3em",fontFamily:"'Montserrat', sans-serif"}}>KAIHA</div><div style={{color:"#666",fontSize:"8px",letterSpacing:"0.2em"}}>© HADI SUKKUR 2026</div></div></div><div style={{marginTop:"20px"}}>{[{l:"Home",v:"home"},{l:"Shop",v:"shop"},{l:"Reels",v:"reels"},{l:"Bag",v:"bag"},{l:"Profile",v:"profile"},{l:"All Items",v:"all"}].map((it:any)=><div key={it.l} onClick={()=>{setPg(it.v);setMn(false)}} style={{padding:"14px",borderRadius:"10px",background:pg===it.v?"#1a1a1a":"transparent",border:"1px solid #222",marginTop:"10px",color:pg===it.v?"#D4B78F":"#ccc"}}>{it.l}</div>)}</div></div></div>}
</div>
</div>
);
  }
