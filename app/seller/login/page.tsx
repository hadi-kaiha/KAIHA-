...
// SECURE LOGIN - SHOP + PASSWORD CHECK
const doLogin=async()=>{
  if(!shop||!pass) return alert("Shop Name + Password likho!");
  setLoading(true);
  const {data,error}=await supabase.from("sellers").select("*").eq("shop_name",shop.trim()).eq("password",pass).single();
  setLoading(false);
  if(error||!data) return alert("❌ Wrong Shop Name ya Password! Koi aur tumhari shop login nahi kar sakta");
  localStorage.setItem("shop_name", data.shop_name);
  localStorage.setItem("seller_phone", data.phone);
  alert("✅ Login Success "+data.shop_name);
  location.href="/seller?shop="+encodeURIComponent(data.shop_name);
};
...
const doRegister=async()=>{
 ...
 setLoading(true);
 try{
   // Pehle check karo shop exist karti hai?
   const {data:exists}=await supabase.from("sellers").select("shop_name").eq("shop_name",shop.trim()).single();
   if(exists) { setLoading(false); return alert("❌ Ye Shop Name pehle se hai! Login karo"); }
   const {error}=await supabase.from("sellers").insert({
     shop_name:shop.trim(), owner_name:form.owner, phone:form.phone, whatsapp:form.whatsapp||form.phone, gmail:form.gmail, easypaisa:form.easypaisa, jazzcash:form.jazzcash, address:form.address, password:form.password, created_at: new Date().toISOString()
   });
   if(error) throw error;
   localStorage.setItem("shop_name", shop.trim());
   alert("✅ Shop Registered! Ab login hoga");
...
{mode==="LOGIN"?(
<div>
<input value={shop} onChange={e=>setShop(e.target.value)} placeholder="Shop Name" style={{width:"100%",marginTop:"16px",padding:"12px",background:"#141414",border:"1px solid #D4B78F66",borderRadius:"8px",color:"#fff"}}/>
...
<div style={{fontSize:"9px",color:"#666",marginTop:"10px",textAlign:"center"}}>Sirf Shop Owner jiska password sahi hai wo hi login kar sakta hai. Dusra koi bhi shop naam likh ke login nahi kar sakta. Owner: 03320821575</div>
</div>
):(
<div>
<input value={shop} onChange={e=>setShop(e.target.value)} placeholder="Shop Name * Unique" style={{width:"100%",marginTop:"14px",padding:"12px",background:"#141414",border:"1px solid #D4B78F66",borderRadius:"8px",color:"#fff"}}/>
...
<button onClick={doRegister} disabled={loading} style={{width:"100%",marginTop:"14px",background:loading?"#222":"#D4B78F",color:"#000",padding:"14px",borderRadius:"999px",border:"none",fontWeight:"900"}}>{loading?"REGISTERING...":"REGISTER →"}</button>
</div>
)}
