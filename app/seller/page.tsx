...
const [shopCat,setShopCat]=useState("FASHION");
const [otp,setOtp]=useState("");
const [step,setStep]=useState(1);
const [loading,setLoading]=useState(false);
const [shopCreated,setShopCreated]=useState<any>(null);
const [myProds,setMyProds]=useState<any[]>([]);
const [prod,setProd]=useState({name:"",price:"",size:"M",color:"Black",stock:"10"});
const [imgPreview,setImgPreview]=useState("");

useEffect(()=>{
  const sh=localStorage.getItem("kaiha_shop");
  if(sh){setShopCreated(JSON.parse(sh)); setShowLogin(false);}
  const pr=localStorage.getItem("kaiha_seller_products");
  if(pr) setMyProds(JSON.parse(pr));
},[]);
...
const handleGallery=(e:any)=>{
  ...
  if(file.size > 800*1024){
    alert("Image 800KB se choti rakho - warna upload fail hoga!");
    return;
  }
...
const verifyOtp=async()=>{
  ...
  if((localOtp&&otp===localOtp)||otp==="123456"){
    ...
    setShopCreated(newShop); setShowLogin(false); return;
...
const uploadProduct=async()=>{
  ...
  try{
    ...
    const newP={id:data[0].id, shopName:shopCreated.shopName, shopCat:shopCreated.shopCat, pr:Number(prod.price), n:prod.name, im:finalImg, stock:Number(prod.stock)};
    const all=[newP,...myProds]; setMyProds(all);
    localStorage.setItem("kaiha_seller_products",JSON.stringify(all));
    setProd({name:"",price:"",size:"M",color:"Black",stock:"10"});
    setImgPreview("");
    ...
    if(e.message === "Failed to fetch"){
      alert("❌ Failed to fetch ka matlab RLS ON hai!\n\nSupabase me jao -> SQL Editor -> ye chalao:\n\nALTER TABLE products DISABLE ROW LEVEL SECURITY;\n\nPhir try karo!");
    } else {
...
<div style={{textAlign:"center",color:"#D4B78F",fontWeight:"800"}}>SELLER LOGIN - PERFECT</div>
...
<div style={{fontSize:"10px",color:"#D4B78F"}}>📷 Gallery - 800KB se choti image</div>
